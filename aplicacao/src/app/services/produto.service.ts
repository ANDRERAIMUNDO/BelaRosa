...
export class ProdutoService {
    ...
    getSmallImage(id: string) : Observable<any> {
        let url = `${API_CONFIG.bukectBaseUrl}/prod${id}-small.png`;
        return this.httpCliente.get(url, {responseType:'blob'});
    }

    getProdutoImage(id: string): Observable<any> {
        let url = `${API_CONFIG.bukectBaseUrl}/pro${id}.png`;
        return this.httpCliente.get(url, {responseType:'blob'});
    }

    getProdutoImageFirebase(id: string): Observable<any> {
        let url = `https://firebasestorage.googleapis.com/v0/b/bela-rosa-storage-firebase.appspot.com/o/prod%2Fpro${id}.png?alt=media&amp;token=null`;
        return this.httpCliente.get(url, {responseType:'blob'});
    }
}